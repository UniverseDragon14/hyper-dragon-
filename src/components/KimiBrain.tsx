...
const MEMORY_KEY = 'NOVA_KIMI_MEMORY_V1';

export const KimiBrain: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('NOVA memory load failed', error);
    }
  }, []);
  ...
  useEffect(() => {
    fetch('/api/health', { cache: 'no-store' })
      ...
      .catch(() => {
        setHealth({ brain_online: false, ai_provider: 'offline', model: 'unreachable' });
      });
  }, []);
  ...
  const sendMessage = useCallback(async (rawText: string) => {
    ...
    try {
      const history = nextMessages.slice(-12, -1).map((message) => ({
        role: message.role,
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: isDeepMode ? `[DEEP_BRAIN_MODE] ${text}` : text,
          history,
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data?.text || data?.error || `HTTP ${response.status}`);
      }

      setHealth((previous) => ({
        ...previous,
        brain_online: true,
        ai_provider: data?.provider || previous.ai_provider || 'kimi',
        model: data?.model || previous.model || 'kimi-k3',
      }));
      ...
      setMessages((previous) => [
        ...
        {
          role: 'assistant',
          text: data?.text || 'NOVA brain returned an empty response.',
          timestamp: Date.now(),
        },
      ]);
    } catch (error: any) {
      setHealth((previous) => ({ ...previous, brain_online: false }));
      setMessages((previous) => [
        ...previous.slice(-39),
        {
          role: 'assistant',
          text: String(error?.message || 'NOVA brain connection failed.'),
          timestamp: Date.now(),
        },
      ]);
    } finally {
  ...
  useEffect(() => {
    ...
    recognition.onerror = () => {
      ...
      setMessages((previous) => [
        ...
        {
          role: 'assistant',
          text: 'Voice signal fail aachu. Type pannunga, NOVA ready.',
          timestamp: Date.now(),
        },
      ]);
    };
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) {
      setMessages((previous) => [
        ...
        {
          role: 'assistant',
          text: 'இந்த browser voice recognition support pannala. Type input use pannunga.',
          timestamp: Date.now(),
        },
      ]);
      return;
  ...
  const clearMemory = () => {
    if (!window.confirm('NOVA local chat memory clear panna ok-aa?')) return;
    localStorage.removeItem(MEMORY_KEY);
    setMessages([]);
  };

  const modelLabel = String(health.model || 'kimi-k3').toUpperCase();
  ...
  return (
    <section className="glass-panel p-4 flex flex-col gap-4 h-full relative overflow-hidden">
      <header className="flex items-center justify-between gap-3 z-10">
        <div>
          <h2 className="micro-label flex items-center gap-2 text-[#00FFFF]">
            <BrainCircuit className="w-4 h-4" /> NOVAKUTTY_BRAIN
          </h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title="Toggle deep brain prompt mode"
          >
            {isDeepMode ? 'DEEP_MODE' : 'FAST_MODE'}
          </button>

          <div className="flex items-center gap-1">
            <Database
              className={`w-3 h-3 ${memoryStatus === 'SAVING' ? 'text-[#FF3300] animate-pulse' : 'text-[#444]'}`}
            />
      ...
      <div
        ref={scrollRef}
        className="flex-1 min-h-[260px] bg-black border border-[#222] rounded p-3 overflow-y-auto font-mono text-[11px] space-y-4"
      >
        <AnimatePresence initial={false}>
          {messages.length === 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-[#444] text-center mt-10 italic"
            >
              Kimi-powered NOVA brain ready. கட்டளைகளுக்காக காத்திருக்கிறேன், Aslam.
            </motion.div>
          )}
          ...
          {messages.map((message, index) => (
            ...
            >
              ...
              <div
                className={`max-w-[82%] p-2 rounded border relative group whitespace-pre-wrap ${
                  message.role === 'user'
      ...
      <div className="flex gap-2 z-10">
        ...
        <input
          ...
          onKeyDown={(event) => {
            if (event.key === 'Enter') void sendMessage(inputText);
          }}
          placeholder="NOVA-kitta கேளுங்கள்..."
          maxLength={4000}
